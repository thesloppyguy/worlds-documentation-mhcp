...
💡 _Tip: Keep filenames short and simple to avoid errors during import._
...
👉 _Tip: This step ensures the FBX will carry the correct material names into Horizon Worlds. Skipping it can cause materials to import incorrectly._
...
2. Open the world where you want to use the asset.
   - ⚠️ _Tip: Always import into the world where the asset will actually be used, so it’s available right away._
...
4. Select your **FBX file** _and_ all the necessary **textures**.
   - If the files are in different folders, you can add them one by one.
   - ⚠️ Don’t click **Import** until _all files_ (FBX + textures) are listed in the Import Model(s) window.
